...
extern void vUartIntrHandler(void *CallBackRef, uint32_t event, uint32_t event_data);
...
int xUartPs1Init(XUartPs* p_XUartPsInst, XScuGic* p_xInterruptController)
{
	...
	* -------------------------------------------------------------------- */
	...
	XUartPs_SetHandler(p_XUartPsInst, (XUartPs_Handler)vUartIntrHandler, p_XUartPsInst);
